...
class OutboxPublishWorkerTest {
    ...
    void finalizeOne_marksPublished_onAck() {
        ...
        SendResult<String, Object> sendResult = new SendResult(null,
                new RecordMetadata(new TopicPartition("payment-initiated", 0), 0L, 0, 0L, 0, 0));
        when(kafkaTemplate.send(eq("payment-initiated"), eq(row.getAggregateId()), any()))
    ...
    @Test
    void finalizeOne_rowMissing_skipsGracefully() {
        UUID missing = UUID.randomUUID();
        when(repository.findOneById(missing)).thenReturn(null);
        worker.finalizeOne(missing, 1000L, 3);
    }

    @Test
    void rescueStuckPublishing_delegates() {
        when(repository.rescueStuckPublishing(60)).thenReturn(3);
        assertThat(worker.rescueStuckPublishing(60)).isEqualTo(3);
    }
    ...
    private OutboxEvent newPendingRow() {
        ...
        e.setEventType(PaymentInitiatedEvent.class.getName());
        e.setTopic("payment-initiated");
        e.setPayload(asJson(PaymentInitiatedEvent.of(txId, 1L, 2L, new BigDecimal("1.00"), "USD", TransactionStatus.PENDING)));
