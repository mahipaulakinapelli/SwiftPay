...
/** Transactional half of the ledger's outbox relay. See gateway counterpart for the contract. */
@Component
public class OutboxPublishWorker {
     ...
     * Aliases legacy {@code com.swiftpay.common.event.*} FQNs (written before the
     * poly-repo refactor) to their local {@code com.swiftpay.ledger.event.*} replicas
    ...
    private static Class<?> resolveEventClass(String eventType) throws ClassNotFoundException {
        if (eventType != null && eventType.startsWith("com.swiftpay.common.event.")) {
            String local = "com.swiftpay.ledger.event." + eventType.substring("com.swiftpay.common.event.".length());
            try {
